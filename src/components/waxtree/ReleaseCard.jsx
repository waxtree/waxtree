    ...
    <article data-release-key={group.key} className="flex items-start gap-3 rounded-[10px] border border-border bg-card px-[14px] py-[10px] transition-colors hover:border-[color-mix(in_srgb,var(--primary)_35%,var(--border))] max-sm:flex-wrap max-sm:gap-x-2.5 max-sm:gap-y-2 max-sm:px-3 max-sm:py-2.5">
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-[5px] max-sm:basis-full">
        {tracks.map((track, trackIndex) => (
          <TrackRow key={track.id} track={track} node={node} isLabel={isLabel} primaryArtist={primaryArtist} state={state} actions={actions} playlistOpen={openPlaylist === track.id} setPlaylistOpen={open => setOpenPlaylist(open ? track.id : null)} hardwaxUrl={hardwax?.url} releaseArtist={releaseArtist} releaseTitle={releaseTitle} releaseLabel={releaseLabel} catno={first.catno} trackIndex={trackIndex} releaseTrackCount={tracks.length} />
        ))}
      </div>
