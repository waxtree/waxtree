...
export const TrackRow = ({ track, node, isLabel, primaryArtist, state, actions, playlistOpen, setPlaylistOpen, hardwaxUrl, releaseArtist, releaseTitle, releaseLabel, catno, trackIndex, releaseTrackCount }) => {
  ...
  // Playback source resolution — see useAudioPreviewSource's own comment
  // for the full priority order (Bandcamp -> Deezer -> YouTube -> record
  // stores). Shared with QueueRow (playlists/likes), which reconstructs
  // the same ctx fields from a bare track object instead of a live node.
  const { audioPreview, resolvedVideo, discogsVideo, hasPlayable, playBest } = useAudioPreviewSource(track, {
    artist,
    labelNameForSearch: isLabel ? node.name : track.label,
    releaseArtist, releaseTitle, releaseLabel, catno, trackIndex, releaseTrackCount, hardwaxUrl,
  }, actions);
