    ...
    <Modal title="♥ My Likes" close={close} maxWidth="620px">
      {tracks.length ? Object.entries(groups).sort((a, b) => b[1].length - a[1].length).map(([genre, items]) => (
        <div key={genre}>
          ...
          {/* Same as PlaylistsModal — QueueRow resolves and plays its own
              best source, no close() here either: browsing/playing
              several liked tracks in a row shouldn't kick the user back
              out to the tree each time. */}
          {items.map(track => <QueueRow key={`${genre}-${track.id}`} track={track} actions={actions} onRemove={() => actions.toggleLike(track.id)} />)}
        </div>
